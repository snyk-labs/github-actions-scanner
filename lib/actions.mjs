...
async function fileExists(owner, repo, path, ref = "main") {
...
class Action {
  ...
  static async fromUrl(url) {
    ...
    if (!foundrepo) return;
    let actionPath = "action.yml";
    if (!await fileExists(owner, repo, actionPath, ref)) {
  ...
  static async fromUses(repo, uses) {
    if (uses.startsWith('.')) {
      ...
      let path = join(uses, "action.yml");
      if (!await fileExists(repo.owner, repo.repo, path)) {
        path = join(uses, "action.yaml");
      }
      ...
      let actionPath = join(subPath, "action.yml");
      if (!await fileExists(repo.owner, repo.repo, actionPath)) {
        actionPath = join(subPath, "action.yaml");
      }
  ...
  async parsedContent() {
    ...
    if (content == null) {
      logger.warning(`Can't extract content: ${this.name} doesn't exist.`);
      return this._actionContent;
    }

    try {
      this._actionContent = YAML.parse(content);
    } catch (error) {
